...
void orbisGlDestroyProgram(GLuint program_id)
{
	...
	if(orbisGlConf && program_id>0)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBIS_GL] glDeleteProgram failed: 0x%08X\n", ret);
			}
		}
	}
}
	...
	if(orbisGlConf && shader_id>0)
	{
		...
		{		
			...
			if(log_length)
			{
				...
				if(ret)
				{
					debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glGetShaderInfoLog failed: 0x%08X\n",ret);
					return;
				}
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] shader compilation with log:\n%s\n",log_buffer);
			}
			else
			{
				debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] shader compilantion no log\n");
			}
		}
	}
}
	...
	if(orbisGlConf && program_id>0)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			if(log_length)
			{
				...
				if(ret)
				{
					debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glGetProgramInfoLog failed: 0x%08X\n",ret);
					return;
				}
				debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] program link with log:\n%s\n",log_buffer);
			}
			else
			{
				debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] program link no log\n");
			}
		}
	}
}
GLuint orbisGlCompileShader(const GLenum type,const GLchar* source) 
{
	...
	if(orbisGlConf)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			{
				ret=glGetError();
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glCreateShader failed: 0x%08X\n",ret);
				return 0;
			}
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glShaderSource failed: 0x%08X\n",ret);
			...
			if(ret)
			{	
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glCompileShader failed: 0x%08X\n",ret);
				return 0;
			}
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glGetShaderiv failed: 0x%08X\n",ret);
				return 0;
			}
			if (!compile_status)
			{
					
					debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] shader compilation failed\n");
					orbisGlShaderLog(shader_id);
					return 0;
			}
			debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] shader compilation shader_id=%d done\n",shader_id);
			orbisGlShaderLog(shader_id);
			return shader_id;
		}
	}
	debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] orbisGl is not initialized\n");
...
GLuint orbisGlLinkProgram(const GLuint vertex_shader,const GLuint fragment_shader)
{
	...
	if(orbisGlConf)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			{
				ret=glGetError();
				debugNetPrintf(DEBUGNET_ERROR,"[ORBIS_GL] glCreateProgram failed: 0x%08X\n", ret);
				return 0;
			}
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glAttachShader(vertex_shader) failed: 0x%08X\n",ret);
				return 0;
			}
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glAttachShader(fragment_shader) failed: 0x%08X\n",ret);
				return 0;
			}
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glLinkProgram() failed: 0x%08X\n",ret);
				return 0;
			}
			glGetProgramiv(program_id, GL_LINK_STATUS,&link_status);
			if (!link_status)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] program link failed\n");
				orbisGlProgramLog(program_id);
				return 0;
			}
			debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] link program_id=%d done\n",program_id);
			orbisGlProgramLog(program_id);
			return program_id;
		}
	}
...
{
    debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] orbisGlLoadShaders %s %s\n",vertexShaderFilename,fragmentShaderFilename);
	char * vsSource=(char *)orbisNfsGetFileContent(vertexShaderFilename);
	if(vsSource==NULL)
	{
	    debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] can't open vertex shader at %s\n",vertexShaderFilename);
		return 0;
	}
	GLuint vs=orbisGlCompileShader(GL_VERTEX_SHADER,vsSource);
	if(vs==0)
	{
	    debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] can't compile vertex shader at %s\n",vertexShaderFilename);
		return 0;
	}
	free(vsSource);
	...
	if(fsSource==NULL)
	{
	    debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] can't open fragment shader at %s\n",vertexShaderFilename);
		return 0;
	}
	GLuint fs=orbisGlCompileShader(GL_FRAGMENT_SHADER,fsSource);
	if(fs==0)
	{
	    debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] can't compile fragment shader at %s\n",vertexShaderFilename);
	}
	free(fsSource);
	GLuint po=orbisGlLinkProgram(vs,fs);
	if(po==0)
	{
	    debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] can't link program with vertex shader %d and fragment shader %d\n",vs,fs);
		return 0;
	}
	glDeleteShader(vs);
	glDeleteShader(fs);
...
void orbisGlDestroyTexture(GLuint texture_id)
{
	...
	if(orbisGlConf)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			if(ret)
			{
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] glDeleteTextures failed: 0x%08X\n",ret);
			}
		}
	}
}
void orbisGlDestroyDisplay()
{
	...
	if(orbisGlConf)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			{
				ret=eglGetError();
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglTerminate failed: 0x%08X\n",ret);
			}
...
void orbisGlDestroySurface()
{
	...
	if(orbisGlConf)
	{
		if(orbisGlConf->orbisgl_initialized==1)
		{
			...
			{
				ret=eglGetError();
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglDestroySurface failed: 0x%08X\n",ret);
			}
...
void orbisGlDestroyContext()
{
	...
	if(orbisGlConf)
	{
		...
		{		
			...
			{
				ret=eglGetError();
				debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglDestroyContext failed: 0x%08X\n",ret);
			}
...
void orbisGlFinish()
{
	if(orbisgl_external_conf!=1)
	{
		...
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] liborbisGl finished\n");
	}
}
...
int orbisGlInitWithConf(OrbisGlConfig *conf)
{
	...
	if(ret)
	{
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] liborbisGl already initialized using configuration external\n");
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] orbisgl_initialized=%d\n",orbisGlConf->orbisgl_initialized);
		debugNetPrintf(DEBUGNET_DEBUG,"[ORBISGL] ready to have a lot of fun...\n");
		return orbisGlConf->orbisgl_initialized;
	}
	else
	{
...
int orbisGlInit(unsigned int width,unsigned int height)
{
	...
	if (orbisGlConf->orbisgl_initialized==1) 
	{
		debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] liborbisGl is already initialized!\n");
		return orbisGlConf->orbisgl_initialized;
	}
	...
	if(!ret) 
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] scePigletSetConfigurationVSH failed 0x%08X.\n",ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] scePigletSetConfigurationVSH success.\n");
	
	orbisGlConf->display=eglGetDisplay(EGL_DEFAULT_DISPLAY);
	if(orbisGlConf->display==EGL_NO_DISPLAY) 
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglGetDisplay failed.\n");
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglGetDisplay success.\n");
	
	ret=eglInitialize(orbisGlConf->display,&major,&minor);
	if(!ret)
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglInitialize failed: 0x%08X\n",ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] EGL version major:%d, minor:%d\n",major,minor);
	
	ret=eglBindAPI(EGL_OPENGL_ES_API);
	if(!ret)
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglBindAPI failed: 0x%08X\n",ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglBindAPI success.\n");
	
	ret=eglSwapInterval(orbisGlConf->display,0);
	if(!ret)
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglSwapInterval failed: 0x%08X\n",ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglSwapInterval success.\n");
	
	ret=eglChooseConfig(orbisGlConf->display,attribs,&orbisGlConf->config,1,&orbisGlConf->num_configs);
	if(!ret)
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglChooseConfig failed: 0x%08X\n",ret);
		return -1;
	}
	if (orbisGlConf->num_configs!=1)
	{
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] No available configuration found.\n");
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglChooseConfig success.\n");
	...
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglCreateWindowSurface failed: 0x%08X\n",ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglCreateWindowSurface success.\n");
	...
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglCreateContext failed: 0x%08X\n", ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglCreateContext success.\n");

	ret=eglMakeCurrent(orbisGlConf->display,orbisGlConf->surface,orbisGlConf->surface,orbisGlConf->context);
	if(!ret) 
	{
		ret=eglGetError();
		debugNetPrintf(DEBUGNET_ERROR,"[ORBISGL] eglMakeCurrent failed: 0x%08X\n",ret);
		return -1;
	}
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] eglMakeCurrent success.\n");
	const char *gl_exts = (char *) glGetString(GL_EXTENSIONS);
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] GL_VENDOR:   \"%s\"\n", glGetString(GL_VENDOR));
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] GL_VERSION:  \"%s\"\n", glGetString(GL_VERSION));
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] GL_RENDERER: \"%s\"\n", glGetString(GL_RENDERER));
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] SL_VERSION:  \"%s\"\n", glGetString(GL_SHADING_LANGUAGE_VERSION));
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] GL_EXTs:     \"%s\"\n", gl_exts);
	orbisGlConf->orbisgl_initialized=1;
	debugNetPrintf(DEBUGNET_INFO,"[ORBISGL] liborbisGl initialized\n");
